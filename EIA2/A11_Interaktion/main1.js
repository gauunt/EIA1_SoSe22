...
(function (L11_InteraktionLuftfahrt) {
    ...
    L11_InteraktionLuftfahrt.handleLoad = handleLoad;
    function create() {
        for (let i = 0; i < 7; i++) {
            let bug = new L11_InteraktionLuftfahrt.Bug(1.0);
            moveables.push(bug);
        }
        for (let i = 0; i < 5; i++) {
            let paraglider = new L11_InteraktionLuftfahrt.Paraglider(1.0);
            moveables.push(paraglider);
        }
        for (let i = 0; i < 5; i++) {
            let läufer = new L11_InteraktionLuftfahrt.Läufer(1.0);
            moveables.push(läufer);
        }
        for (let i = 0; i < 2; i++) {
            let ballon = new L11_InteraktionLuftfahrt.Ballon(1.0);
            moveables.push(ballon);
        }
        for (let i = 0; i < 5; i++) {
            let lander = new L11_InteraktionLuftfahrt.Lander(1.0);
    ...
    // Sprechblase
    function drawSpeechbubble(_position) {
        crc2.save();
        crc2.translate(_position.x, _position.y);
        crc2.beginPath();
    ...
    function handleMausklick(event) {
    ...
    function handleKeyDown(event) {
        if (event.key === "ArrowLeft") {
            for (let moveable of moveables) {
                if (moveable instanceof L11_InteraktionLuftfahrt.Moveable) {
